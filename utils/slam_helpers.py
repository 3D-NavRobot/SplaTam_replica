...
def transform_to_frame12(params, time_idx, gaussians_grad, camera_grad):
    """
    Function to transform Isotropic or Anisotropic Gaussians from world frame to camera frame.
    ...
    # Get Frame Camera Pose
    if camera_grad:
        # pull out the quaternion for this frame (shape [4]) and normalize
        cam_rot  = F.normalize(params['cam_unnorm_rots'][time_idx], dim=-1)
        cam_tran = params['cam_trans'][time_idx]
    else:
        # same thing, but detached
        cam_rot  = F.normalize(params['cam_unnorm_rots'][time_idx], dim=-1).detach()
        cam_tran = params['cam_trans'][time_idx].detach()

    # build the 4×4 w2c matrix
    rel_w2c = torch.eye(4, device=cam_tran.device, dtype=cam_tran.dtype)
    rel_w2c[:3, :3] = build_rotation(cam_rot.unsqueeze(0))[0]
    rel_w2c[:3, 3]  = cam_tran
