...
def pose_refinement_loss(params, curr_data, variables, iter_time_idx, loss_weights):
    # Transform Gaussians to the current frame
    transformed_gaussians = transform_to_frame(params, iter_time_idx, gaussians_grad=True, camera_grad=True)

    # Render depth and silhouette
    depth_sil_rendervar = transformed_params2depthplussilhouette(params, curr_data['w2c'], transformed_gaussians)
    depth_sil, _, _, = Renderer(raster_settings=curr_data['cam'])(**depth_sil_rendervar)
    rendered_depth = depth_sil[0, :, :].unsqueeze(0)
    rendered_silhouette = depth_sil[1, :, :]

    # Compute Depth Loss
    depth_loss = torch.abs(curr_data['depth'] - rendered_depth).mean()

    # Compute Silhouette Loss
    sil_loss = torch.nn.functional.binary_cross_entropy(rendered_silhouette, curr_data['silhouette'])

    # Combine Losses
    total_loss = loss_weights['depth'] * depth_loss + loss_weights['silhouette'] * sil_loss
    return total_loss

def refine_pose(params, curr_data, variables, iter_time_idx, optimizer, loss_weights, num_iters=100):
    for _ in range(num_iters):
        optimizer.zero_grad()
        loss = pose_refinement_loss(params, curr_data, variables, iter_time_idx, loss_weights)
        loss.backward()
        optimizer.step()
    return params
...
def rgbd_slam(config: dict):
    ...
    for time_idx in tqdm(range(checkpoint_time_idx, num_frames)):
        ...
        if time_idx > 0 and not config['tracking']['use_gt_poses']:
            ...
            #refine poses
            params = refine_pose(params, tracking_curr_data, variables, time_idx, optimizer, config['tracking']['loss_weights'])
