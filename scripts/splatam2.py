...
def plan_path_astar(means2d, start_pt, goal_pt, k=8):
    """
    means2d:  (N,2) numpy array of Gaussian centers in floor-plane
    start_pt, goal_pt:  2-vectors
    returns: list of indices into means2d forming the path
    """
    # build kNN graph
    tree = cKDTree(means2d)
    dists, neighs = tree.query(means2d, k=k+1)  # includes self at neighs[:,0]
    neighs = neighs[:,1:]   # drop self
    dists = dists[:,1:]

    N = len(means2d)
    # find nearest Gaussians to start/goal
    _, sidx = tree.query(start_pt)
    _, gidx = tree.query(goal_pt)

    # A* on this small graph
    open_set = [(0.0, sidx)]
    g_score = np.full(N, np.inf)
    g_score[sidx] = 0.0
    f_score = np.full(N, np.inf)
    f_score[sidx] = np.linalg.norm(means2d[sidx] - goal_pt)
    ...
    while open_set:
        ...
        for vi, v in enumerate(neighs[u]):
            tentative_g = g_score[u] + dists[u,vi]
            if tentative_g < g_score[v]:
                came_from[v] = u
                g_score[v] = tentative_g
                f_score[v] = tentative_g + np.linalg.norm(means2d[v] - goal_pt)
                heapq.heappush(open_set, (f_score[v], v))

    return []   # no path found


def prune_by_path(params, variables, path_indices, buffer=0.2):
    """
    ...
    # compute for each Gaussian its min distance to the polyline
    # We just do min over vertices; we could do true segment‐to‐point distance
    dists = np.linalg.norm(
        means2d[:,None,:] - path_pts[None,:,:],
        axis=2
    )  # shape (N, len(path))
    keep_mask = dists.min(axis=1) <= buffer

    # now filter all per-Gaussian parameters
    gaussian_keys = ['means3D','rgb_colors','unnorm_rotations','logit_opacities','log_scales']
    for k in gaussian_keys:
        tensor = params[k]
        kept = tensor[keep_mask]
        params[k] = torch.nn.Parameter(kept.contiguous().requires_grad_())

    # filter your variables too
    for k in ['max_2D_radius','means2D_gradient_accum','denom','timestep']:
        variables[k] = variables[k][keep_mask].clone()
