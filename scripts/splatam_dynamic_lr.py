...
# Dynamic learning rate decay function
# The learning rate is linearly interpolated between lr_init and lr_final
def get_exponential_lr_func(lr_init, lr_final, lr_delay_steps=0, lr_delay_mult=1.0, max_steps=1000):
    """
...
def update_learning_rate(optimizer, lr_scheduler, iteration):
    """Update learning rate for given optimizer and iteration step"""
    for param_group in optimizer.param_groups:
        if param_group["name"] == "means3D":
            # This is just one example - you might want to apply to other parameter groups too
            lr = lr_scheduler(iteration)
            param_group['lr'] = lr
            return lr  # Return the new learning rate
    return 0.0  # Return 0 if no update was made
...
def rgbd_slam(config: dict):
    ...
    for time_idx in tqdm(range(checkpoint_time_idx, num_frames)):
        ...
        if time_idx > 0 and not config['tracking']['use_gt_poses']:
            ...
            # Initialize the learning rate scheduler for tracking
            lr_init = config['tracking']['lrs']['means3D']
            lr_final = lr_init * 0.1  # Set final lr to 10% of initial lr, or configure in your config
            max_steps = config['tracking']['num_iters']
            tracking_lr_scheduler = get_exponential_lr_func(
                lr_init=lr_init,
                lr_final=lr_final,
                max_steps=max_steps
            )
            ...
            while True:
                ...
                current_lr = update_learning_rate(optimizer, tracking_lr_scheduler, iter+1)
                ...
                if config['use_wandb']:
                    ...
                    wandb_run.log({"Tracking/Learning Rate": current_lr}, step=wandb_tracking_step)
                    
                # Rest of your tracking loop...              
                # Backprop
                loss.backward()
                # Optimizer Update
                optimizer.step()
        ...
        if time_idx == 0 or (time_idx+1) % config['map_every'] == 0:
            ...
            # Initialize learning rate scheduler for mapping
            mapping_lr_scheduler = get_exponential_lr_func(
                lr_init=config['mapping']['lrs']['means3D'],
                lr_final=config.get('mapping', {}).get('lrs_means3D_final', config['mapping']['lrs']['means3D'] * 0.1),  # Default to 10% if not specified
                lr_delay_mult=config.get('mapping', {}).get('lr_delay_mult', 1.0),
                max_steps=config['mapping']['num_iters']
            )
            ...
            for iter in range(num_iters_mapping):
                ...
                current_lr = update_learning_rate(optimizer, mapping_lr_scheduler, iter+1)
                if config['use_wandb']:
                    # Report Loss
                    wandb_mapping_step = report_loss(losses, wandb_run, wandb_mapping_step, mapping=True)
                    wandb_run.log({"Mapping/Learning Rate": current_lr}, step=wandb_mapping_step)
                # Backprop
                loss.backward()
                with torch.no_grad():
                    # Prune Gaussians
