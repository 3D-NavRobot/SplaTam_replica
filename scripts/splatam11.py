...
def adaptive_prune_gaussians(params, variables, prune_ratio):
    ...
    # find the k-th smallest radius
    threshold = torch.kthvalue(radii, k).values
    keep = radii > threshold

    # apply mask to every per-Gaussian tensor in params + variables
    for d in params:
        params[d] = torch.nn.Parameter(params[d][keep].clone().requires_grad_(True))
    for v in ['max_2D_radius', 'means2D_gradient_accum', 'denom', 'timestep']:
        variables[v] = variables[v][keep].clone()
